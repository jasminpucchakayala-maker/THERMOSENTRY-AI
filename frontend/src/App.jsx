...
function App() {
  ...
  return <div className={`mission-shell ${nightMode ? '' : 'day-mode'}`}>
    ...
    <main className="mission-stage">
      ...
      {activeNav !== 'Overview' && <NavigationPanel activeNav={activeNav} anomalies={anomalies} onSelect={handleMarkerSelect} firmsError={firmsError} />}
    </main>
...
function NavigationPanel({ activeNav, anomalies, onSelect, firmsError }) {
  const isLiveDataView = activeNav === 'Live Orbit' || activeNav === 'Thermal Events'
  return <section className="navigation-panel"><div className="navigation-panel-header"><div><span>MISSION WORKSPACE</span><h2>{activeNav}</h2></div><small>{isLiveDataView && !firmsError ? 'LIVE DATA' : 'MODULE NOT CONNECTED'}</small></div>{activeNav === 'Thermal Events' ? <div className="event-list">{anomalies.length ? anomalies.map((event) => <button key={event.id} onClick={() => onSelect(event)}><Flame size={16} /><span><strong>{event.id}</strong><small>{event.latitude.toFixed(4)}, {event.longitude.toFixed(4)} · {event.acquisition_date} {event.acquisition_time}</small></span><ChevronRight size={15} /></button>) : <p>No live FIRMS detections returned for the current query.</p>}</div> : activeNav === 'Live Orbit' ? <div className="workspace-summary"><DataRow label="Satellite" value="NOAA-20 (VIIRS)" accent /><DataRow label="Query scope" value="India bounding box" /><DataRow label="Thermal detections" value={anomalies.length} accent /><DataRow label="Feed status" value={firmsError ? 'Offline' : 'Connected'} /></div> : <div className="workspace-empty"><AlertTriangle size={24} /><strong>{activeNav} is not connected yet</strong><span>This workspace will use the existing backend intelligence modules when they are implemented.</span></div>}</section>
}
